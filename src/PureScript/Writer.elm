module PureScript.Writer exposing (..)

{-| Adapted from stil4m/elm-syntax; see Writer.LICENCE.
...
writeModuleName moduleName =
    string (String.join "." ("Elm" :: moduleName))
...
writeImport { moduleName, moduleAlias, exposingList } =
    breaked
        [ spaced
            ...
            , writeModuleName <|
                Node.value
                    (Maybe.withDefault moduleName moduleAlias)
            ]
        , maybe
            (exposingList
                |> Maybe.map
...
writeTypeAnnotation (Node _ typeAnnotation) =
    case typeAnnotation of
        ...
        Tupled xs ->
            sepBy ( "(", "/\\", ")" ) False (List.map writeTypeAnnotation xs)
...
writeExpression (Node range inner) =
    ...
    case inner of
        ...
        Hex h ->
            writeNum (join [ string "0x", string (Hex.toString h) ])

        Integer i ->
            writeNum (string (String.fromInt i))
        ...
        TupledExpression t ->
            join [ string "(", sepHelper (sepBy ( "", "/\\", "" )) (List.map recurRangeHelper t), string ")" ]
    ...
    let
        ...
        escape =
            if c == '\t' || c == '\'' || c == '\\' then
                ...
                ""
    in
    string ("(_C '" ++ escape ++ String.fromChar c ++ "')")


writeNum : Writer -> Writer
writeNum n =
    join [ string "(_N ", n, string ")" ]
